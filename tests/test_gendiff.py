...
def test_generate_diff():
    for f in test_data:
        ...
        with open('tests/test_data/result_flat.txt', 'r') as f:
            result = f.read().strip()
        assert generate_diff(file1, file2, formatter='stylish') == result
...
def test_generate_diff3():
    for f in test_data2:
        file1, file2 = f
        with open('tests/test_data/result_plain.txt', 'r') as f:
            result = f.read().strip()
        assert generate_diff(file1, file2, formatter='plain') == result
...
def test_generate_diff4():
    for f in test_data2:
        ...
        with open('tests/test_data/result_json.txt', 'r') as f:
            result = f.read().strip()
        assert generate_diff(file1, file2, formatter='json') == result
